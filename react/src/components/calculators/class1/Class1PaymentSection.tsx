...
export default function Class1PaymentSection(props: Class1PaymentSectionProps) {
    ...
    const handleMaxValue = (enteredValue: number) => {
        ...
        else if(getAllowedRows(currentTotalRows) === 0) { // validation for repeat qty if maximum limit is reached
    ...
    useEffect(() => {
        if (rows && rows.length > 0) {
            ...
            rows.forEach((row) => {
                ...
                if(period === '2W'){ // if period is fortnightly
                    lastWeekNumber = rowNumber * 2;
                    firstWeekNumber = lastWeekNumber - 1;
                }
                else if(period === '4W'){ // if period is 4 weekly
                ...
                if(period !== 'W') {
                    const firstWeek = memoizedTaxYearWeeks.find((w) => w.week === firstWeekNumber);
                    startDateOfWeek = firstWeek?.startDateOfWeek;
                    // get week for last week number if period is fortnightly or 4 weekly
                    if(period !== 'M') {
                        const lastWeek = memoizedTaxYearWeeks.find((w) => w.week === lastWeekNumber);
                        endDateOfWeek = lastWeek?.endDateOfWeek;
                    }
                    // if period is monthly
                    else{
                        const yearFirstWeek = memoizedTaxYearWeeks.find((w) => w.week === 1);
                        const yearFirstWeekDate = yearFirstWeek?.startDateOfWeek;
                        // add number of months to year start date
                        startDateOfWeek = moment(yearFirstWeekDate, DATE_FORMAT_DD_MM_YYYY).add(rowNumber-1, 'month').format(DATE_FORMAT_DD_MM_YYYY)
                        // add 30 days to start date to get end date of month
                        endDateOfWeek = moment(startDateOfWeek, DATE_FORMAT_DD_MM_YYYY).add(1, 'month').subtract(1, 'day').format(DATE_FORMAT_DD_MM_YYYY)
                    }
                }

                // to check if period is between start and end date of tax year range
                if (startDateOfWeek && endDateOfWeek && taxYearPeriod && taxYearPeriod.txYears.length > 1) {
                    ...
                    taxYearPeriod.txYears.forEach((ty) => {
                        ...
                        if (
                            moment(fromDate).isBetween(
                                moment(startDateOfWeek, DATE_FORMAT_DD_MM_YYYY).format(DATE_FORMAT_YYYY_MM_DD),
                                moment(endDateOfWeek, DATE_FORMAT_DD_MM_YYYY).format(DATE_FORMAT_YYYY_MM_DD)
                            ) ||
    ...
    const handleClick = useCallback(
        (e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            const allowedRows = getAllowedRows(currentTotalRows);
            if(repeatQty > allowedRows) {
                // don't allow to add more rows
                setIsRepeatAllow(false);
            }
            else if(getAllowedRows(rows.length) === 0) { // validation for repeat qty if maximum limit is reached
                setIsRepeatAllow(false);
                return false;
            }
            else {
            ...
            const selectedIndex = activeRowId ? rows.findIndex((r) => r.id === activeRowId) : rows.length - 1
    ...
    const handleDeleteRow = (e: React.MouseEvent) => {
        ...
        if (activeRowId) {
            ...
            if (getAllowedRows(currentTotalRows) === 0 || repeatQty > getAllowedRows(currentTotalRows)) { // validation for repeat qty if maximum limit is reached
                setIsRepeatAllow(false);
            }
            else {
                setIsRepeatAllow(true);
    ...
    const handleDateInputChange = useCallback((row, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
      <>
        ...
        <div className="container stack-left">
            <div className="container">
                <div className="form-group repeat-button repeat-row">
                    <SecondaryButton label="Repeat row" onClick={handleClick} disabled={!isRepeatAllow} />
                    {` x `}
        ...
        {customRows.length > 0 &&
            <Class1PeriodsSection customRows={customRows} handleDateInputChange={handleDateInputChange} />
        }
